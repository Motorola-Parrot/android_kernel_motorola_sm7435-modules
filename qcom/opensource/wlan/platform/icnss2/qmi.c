...
static int get_moto_sku(void)
{
    ...
    if (rc || !sku_val) {
        icnss_pr_err("get hardware.sku string failed");
        return -ENOMEM;
    }else{
...
static int get_moto_radio(void)
{
    ...
    if (!get_moto_sku()) {
        ...
        {
            icnss_pr_dbg("get radio by sku:%s", radio_ptr);
            return 0;
        } else {
            icnss_pr_dbg("sku&radio not listed!");
        }
    }
...
static int selectFileNameByProduct(struct icnss_priv *plat_priv, char *filename, u32 bdf_type)
{
	int i, num, ret = 0;
	...
	if (strlen(filename) != 0){
        icnss_pr_err("bdf filename=%s", filename);
    }
				   ...
				   u32 filename_len)
{
	...
	if (selectFileNameByProduct(priv, filename, bdf_type) > 0)
		return ret;
	switch (bdf_type) {
	case ICNSS_BDF_ELF:
